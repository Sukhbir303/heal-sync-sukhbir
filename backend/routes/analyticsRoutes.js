...
router.get('/heatmap/:diseaseType', async (req, res) => {
  try {
    ...
    const heatmapData = Object.entries(zoneData).map(([zone, data]) => {
      const riskScore = data.totalTests > 0 
        ? (data.positiveCases / data.totalTests) * 100 
        : 0;
...
router.get('/map/entities', async (req, res) => {
  try {
    ...
    const entities = await Entity.find(filter).select(
      'name entityType zone coordinates currentState'
    ).lean();
